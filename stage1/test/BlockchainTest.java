...
class Block {
    ...
    // Create a new Set to store unique message IDs
    static Set<String> uniqueMessageIDs = new HashSet<>();
    ...
    static Block parseBlock(String strBlock) throws BlockParseException, NoSuchAlgorithmException {
        ...
        // 8. Validate the Message ID and store it
        String messageID = validateMessageId(block, lines, i, uniqueMessageIDs);

        // 9. Validate the Signature and store the decoded signature
        byte[] decodedSignature = validateSignature(block, lines, i + 1);

        // 10. Validate the Public Key
        byte[] decodedPublicKey = validatePublicKey(block, lines, i + 2);

        // Extract the block data
        String blockData = block.blockDataMessage[block.id - 2];

        // Verify the signature with the public key and message ID
        verifySignatureWithPublicKey(decodedSignature, decodedPublicKey, blockData, block.id);

        // 11. Validate the block generation time
        validateGenerationTime(block, lines, i + 3); // Returns the line `N ...`

        // 12. Validate if N increased, decreased or stayed the same
        validateN(block, lines, i + 4);
        ...
        if (block.id > 1 && block.id <= 5) {
            ...
            if (!Arrays.asList(block.blockDataMessage).contains(lines.get(i))) {
                throw new BlockParseException("In Block" + block.id + " the chat message within the Block data "
                        + "should be: " + block.blockDataMessage[block.id] +
...
//
//        MessageDigest digest = MessageDigest.getInstance("SHA-256");
//        String binaryData = block.timestamp + block.blockDataMessage[0];
//
//        // String binaryData = block.id + block.blockDataMessage[0];
//
//        // Create a double SHA-256 hash of the binary data
//
...
//            throw new BlockParseException("The message ID of Block " + block.id + " is incorrect.\nYour program printed: \"" + messageID + "\"\n"
//                    + "Expected Message ID: \"" + calculatedHash + "\"\n" +
//                    "Make sure you are only using the Block's timestamp: \"" + block.timestamp + "\" and the chat message: \"" + block.blockDataMessage[0] + "\" to calculate the message ID.");
//        }
//
//        return msgIdLineIndex;
//    }

    private static String validateMessageId(Block block, List<String> lines, int msgIdLineIndex, Set<String> uniqueMessageIDs) throws BlockParseException {
        if (block.id == 1) {
            // return msgIdLineIndex;
            return null;
        }
        ...
        // Check if the message ID has the correct length for a SHA-256 hash
        if (messageID.length() != 64) {
            throw new BlockParseException("The message ID of Block " + block.id + " has an incorrect length.\nYour program printed: \"" + messageID + "\"\n"
                    + "A valid SHA-256 hash must have a length of 64 characters.");
        }

        // Check if the message ID is unique
        if (uniqueMessageIDs.contains(messageID)) {
            throw new BlockParseException("The message ID of Block " + block.id + " is not unique.\nYour program printed: \"" + messageID + "\"\n"
                    + "And the message ID of Block " + (block.id - 1) + " is: \"" + uniqueMessageIDs.toArray()[uniqueMessageIDs.size() - 1] + "\"\n"
                    + "Make sure you are using the Block's data: \"" + block.blockDataMessage[block.id - 2] + "\" and a random 32-bit signed integer when generating the message ID.");
        }

        // Add the message ID to the set of unique message IDs
...
//    }
    ...
    private static byte[] validateSignature(Block block, List<String> lines, int signatureLineIndex) throws BlockParseException {
        if (block.id == 1) {
            // return signatureLineIndex;
            return null;
        }
...
//    }
    ...
    private static byte[] validatePublicKey(Block block, List<String> lines, int publicKeyLineIndex) throws BlockParseException {
        if (block.id == 1) {
            // return publicKeyLineIndex;
            return null;
        }
...
//    private static void verifySignatureWithPublicKey(byte[] decodedSignature, byte[] decodedPublicKey, String messageID, int blockId) throws BlockParseException {
//        try {
...
//
//            // Update the signature object with the message ID bytes and verify the signature
//            ecdsaSignature.update(messageID.getBytes());
//
//
//            if (!ecdsaSignature.verify(decodedSignature)) {
//                throw new BlockParseException("The signature is not valid for the given message ID and public key in Block " + blockId + "\n" +
//                        "Your Message ID: " + messageID + "\n" +
//                        "Your Signature: " + Base64.getEncoder().encodeToString(decodedSignature) + "\n" +
//                        "Your Public Key: " + Base64.getEncoder().encodeToString(decodedPublicKey) + "\n\n" +
//
//                        "This error occurs when the provided signature does not match the expected signature for the given message ID and public key.\n" +
//                        "Please make sure you're signing the message with the correct private key corresponding to the provided public key, and that the message ID is accurate.\n" +
//                        "Ensure there are no accidental changes or corruption in the data.");
//            }
//
//        } catch (NoSuchAlgorithmException | InvalidKeySpecException | InvalidKeyException | SignatureException e) {
//            throw new BlockParseException("Error validating the signature in Block " + blockId + ": " + e.getMessage());
//        }
//    }

    private static void verifySignatureWithPublicKey(byte[] decodedSignature, byte[] decodedPublicKey, String blockData, int blockId) throws BlockParseException {
        try {
            // Create the EC public key
            X509EncodedKeySpec publicKeySpec = new X509EncodedKeySpec(decodedPublicKey);
            KeyFactory keyFactory = KeyFactory.getInstance("EC");
            ...
            // Update the signature object with the block data bytes and verify the signature
            ecdsaSignature.update(blockData.getBytes());

            // Print the Signature and Public Key as a string here
            // System.out.println("Signature: " + Base64.getEncoder().encodeToString(decodedSignature));
            // System.out.println("Public Key: " + Base64.getEncoder().encodeToString(decodedPublicKey));

            if (!ecdsaSignature.verify(decodedSignature)) {
                throw new BlockParseException("The signature is not valid for the given block data and public key in Block " + blockId + "\n" +
                        "Your Block Data: " + blockData + "\n" +
                        "Your Signature: " + Base64.getEncoder().encodeToString(decodedSignature) + "\n" +
                        "Your Public Key: " + Base64.getEncoder().encodeToString(decodedPublicKey) + "\n\n" +

                        "This error occurs when the provided signature does not match the expected signature for the given block data and public key.\n" +
                        "Please make sure you're signing the message with the correct private key corresponding to the provided public key, and that the block data is accurate.\n" +
                        "Ensure there are no accidental changes or corruption in the data.");
            }
